"""Run the R-104 bounded voiced long-term predictor speech fast gate."""
...
def main() -> None:
    ...
    if channels.shape[1] != 1:
        raise ValueError("R-104 fast gate requires mono speech")
    source = channels[:, 0]
    ...
    (args.output_directory / "voiced-predictive.vpr1").write_bytes(
        candidate.payload
    )
    write_pcm16_channels(
    ...
    write_pcm16_channels(
        args.output_directory / "voiced-predictive-decoded.wav",
        sample_rate,
        candidate.reconstruction.reshape(-1, 1),
    )
    report = {
        "schema": "resonith-voiced-predictive-gate-1",
        "decision": "R-104",
        "status": (
        ...
        "shortlist": [
            {
                ...
                "voiced_block_fraction": record["candidate"].report[
